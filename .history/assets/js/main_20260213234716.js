...
const LOVE_LETTER = [
  'Quỳnh à, anh không muốn viết cho em một bản tuyên ngôn tình yêu thật lớn. Anh chỉ muốn nói vừa đủ, như cách người ta giữ một bức thư trong ngăn bàn: giản dị, rõ ràng, nhưng đọc lại vẫn thấy ấm.',
...
function initAutoMusicGesture() {
  const onceStart = (event) => {
    if (state.hasUserGesture) return;
    state.hasUserGesture = true;

    const target = event?.target;
    const isMusicButton = target && (
      target.closest?.('#music-toggle') ||
      target.closest?.('#play-btn')
    );

    // Let explicit music button clicks control playback themselves
    // to avoid "first click plays then immediately pauses" conflicts.
    if (isMusicButton) {
      if (state.pendingUnmute && state.audio && !state.audio.paused) {
        state.audio.muted = false;
        state.pendingUnmute = false;
        fadeAudioVolume(state.audio, state.audio.volume || 0, 0.2, 320);
      }
      return;
    }
  ...
  window.addEventListener('touchstart', onceStart, { once: true, passive: true });
}

function toggleMusic(forcePlay = null) {
  if (!state.audio) return;

  if (state.pendingUnmute && !state.audio.paused) {
    state.audio.muted = false;
    state.pendingUnmute = false;
    fadeAudioVolume(state.audio, state.audio.volume || 0, 0.2, 420);
    return;
  }
...
function setTrack(trackIndex, keepPlaybackState = true) {
  ...
  const prevVolume = audio.volume || 0.2;
  ...
  if (keepPlaybackState && wasPlaying) {
    ...
    audio.volume = prevMuted ? 0 : Math.min(0.2, prevVolume);
    audio.play().then(() => {
      state.musicPlaying = true;
      if (state.playBtn) state.playBtn.textContent = '⏸';
      if (state.musicToggle) state.musicToggle.classList.add('playing');
      ...
      // Browser may block autoplay; first gesture handler will recover.
    });
  }
}
