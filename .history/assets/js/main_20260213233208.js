...
const LOVE_LETTER = [
  'Quỳnh à, nếu có thể gói tình yêu của anh vào một thứ gì đó để em mang theo, anh sẽ gói vào những đoạn đường: đường từ Hà Nội đi về phía em, đường từ Huế đi qua những ngày dài, và đường từ “suýt nữa” đến “cuối cùng cũng thuộc về nhau”.',
  'Mình quen nhau hơn một năm rồi, đúng không? Có những hôm anh vẫn tự hỏi: làm sao một người có thể bước vào đời mình nhẹ đến vậy, mà rồi khiến mình đổi cả nhịp sống. Một năm trước, mình đã suýt đến được với nhau — gần đến mức anh tưởng chỉ cần với tay là chạm. Rồi vì nhiều lý do, mọi thứ lại lùi lại một bước, như thể ông trời muốn mình học thêm một chút kiên nhẫn để yêu cho đàng hoàng hơn.',
  'Nhưng cái “lùi lại” ấy không phải để mất nhau. Nó giống như một khoảng lặng trong bản nhạc — để khi giai điệu quay lại, mình nghe rõ hơn tim mình đang chọn điều gì. Thời gian tìm hiểu nhau của mình vui lắm, vui theo cái kiểu rất thật: kể nhau nghe những vụn vặt trong ngày, cười vì những câu vô duyên, và dần dần thấy người kia trở thành một thói quen ấm áp.',
  'Rồi đến tháng 10, mình yêu nhau chính thức — không phải bằng một màn pháo hoa ồn ào, mà bằng một lời hứa rất người: “ở lại”. Anh nhớ cảm giác ấy như nhớ một đêm trời trong: mọi thứ bỗng rõ ràng, và anh biết từ nay trái tim anh có địa chỉ.',
  'Phần lớn thời gian mình yêu xa: em ở Huế, anh ở Hà Nội. Có những buổi tối, anh chỉ muốn đi thật nhanh qua những con phố để kịp về nhà, kịp nghe giọng em, kịp đặt một câu “em mệt không?” vào đúng chỗ tim em đang cần. Xa nhau làm mình học cách yêu bằng sự tin tưởng và bằng những điều nhỏ nhất: một tin nhắn đúng lúc, một cuộc gọi dài hơn thường lệ, một cái “nhớ em” không hề cũ.',
  'Những ngày nghỉ dài, những dịp lễ như Tết… mình mới được gặp nhau. Anh thích cái khoảnh khắc “gặp lại” ấy — như thể cả thế giới bỗng dưng im đi để nhường chỗ cho một cái ôm. Mọi khoảng cách lúc đó không còn là địa lý nữa, mà chỉ là một câu chuyện mình đã đi qua để hiểu nhau hơn, thương nhau sâu hơn.',
  'Em biết không, anh không muốn yêu em bằng danh sách lý do, vì tình yêu của anh không nằm trong việc “đếm”. Anh yêu em bằng những đoạn dài của đời sống: bằng những ngày mình không hoàn hảo, bằng những lúc cả hai bối rối, bằng những lần mình chọn nói thật, chọn tử tế, chọn nắm tay nhau thay vì bỏ đi. Anh yêu em bằng cách anh muốn chăm sóc tương lai của mình — nơi em là điều anh trân trọng nhất.',
  'Anh cũng biết mình đã trải qua những khoảng không ổn. Nhưng việc gần đây mình được bên nhau không phải là một sự tình cờ muộn màng; nó là kết quả của việc cả hai đã lớn lên, đã hiểu điều gì là đáng giữ, và đã can đảm chọn nhau.',
  'Nếu tình yêu là một hành trình, anh muốn làm người cùng em đi qua mọi mùa: mùa mình ở xa, mùa mình ở gần, mùa mình bận rộn, mùa mình rảnh rỗi. Anh không hứa cuộc sống chỉ toàn màu hồng, nhưng anh hứa sẽ giữ sự dịu dàng cho em — như cách anh đang giữ em trong lòng bây giờ.',
  'Nên em à, nếu có điều gì anh muốn nói thật dài, thì đó là: cảm ơn em vì đã đến, đã ở, và đã cho anh một “chúng mình” đủ ấm để anh tin vào ngày mai. Và dù ở Huế hay Hà Nội, dù Tết mới gặp hay chỉ kịp nhìn nhau qua màn hình, anh vẫn muốn yêu em theo cách đàng hoàng nhất. 💕',
];
...
const state = {
  ...
  pendingUnmute: false,
  trackPositions: Object.create(null),
...
function initMemoryStage() {
  ...
  if (lineEl) {
    lineEl.textContent = state.memoryStageLines[0];
  }
}
...
function startMemoryStage() {
  ...
  state.memoryStageTimer = setInterval(() => {
    ...
    const lineIndex = state.memoryStageIndex % state.memoryStageLines.length;

    stage.classList.add('memory-fade');
    setTimeout(() => {
      stage.style.backgroundImage = `url("assets/images/${imageList[state.memoryStageIndex]}")`;
      if (lineEl) lineEl.textContent = state.memoryStageLines[lineIndex];
...
function initMelody() {
  ...
  // - If blocked, try muted autoplay then unmute on first gesture
...
function initAutoMusicGesture() {
  const onceStart = () => {
    ...
    if (state.pendingUnmute && state.audio) {
      state.audio.muted = false;
      state.pendingUnmute = false;
      fadeAudioVolume(state.audio, state.audio.volume || 0, 0.2, 520);
      return;
    }

    if (CONFIG.music.length > 0) {
      setTrack(0, true);
      if (state.audio) state.audio.volume = 0.16;
      toggleMusic(true);
    }
  };
...
function toggleMusic(forcePlay = null) {
  ...
  if (shouldPlay) {
    state.audio.volume = state.audio.muted ? 0 : 0.2;
    state.audio.play().then(() => {
      ...
      // Silent fail: autoplay policies vary by browser/device.
      // We'll still start immediately on first gesture (see initAutoMusicGesture).
      if (state.songInfo && state.currentPage === 'page-melody') {
        state.songInfo.textContent = `${CONFIG.music[state.currentTrack].title} — chạm nhẹ để nhạc bật lên 🎵`;
      }
    });
  } else {
    state.audio.pause();
...
function rememberCurrentTrackPosition() {
  if (!state.audio || Number.isNaN(state.audio.currentTime)) return;
  state.trackPositions[state.currentTrack] = state.audio.currentTime;
}

function bindTrackPositionMemory() {
  if (!state.audio) return;
  if (state._trackMemoryBound) return;
  state._trackMemoryBound = true;
  state.audio.addEventListener('timeupdate', () => {
    if (!state.musicPlaying || state.audio.paused) return;
...
function attemptAutoPlayOnLoad() {
  ...
  bindTrackPositionMemory();

  // Ensure we have a src ready.
  if (!state.audio.src) setTrack(0, false);
  ...
  state.audio.play().then(() => {
    ...
    // Fallback: autoplay muted (often allowed), then unmute on first gesture.
    ...
    state.audio.play().then(() => {
      ...
      // Can't autoplay at all; initAutoMusicGesture will handle it.
    });
  });
}
...
function setTrack(trackIndex, keepPlaybackState = true) {
  ...
  // Save current position of the current track before switching.
  rememberCurrentTrackPosition();
  const switchAnchorTime = (!Number.isNaN(state.audio.currentTime) ? state.audio.currentTime : 0) || 0;
  ...
  // Avoid rapid double-switching when users tap fast.
  if (state.switchingTrack) {
    state.currentTrack = targetIndex;
  }
  state.switchingTrack = true;

  const desiredResume = state.trackPositions[targetIndex];
  const resumeHint = (typeof desiredResume === 'number') ? desiredResume : switchAnchorTime;
  ...
  const doSwap = () => {
    state.currentTrack = targetIndex;
    audio.src = `assets/music/${target.file}`;
    audio.load();
    updateSongInfo();
    ...
    const applyResume = () => {
      try {
        ...
        const safe = (Number.isFinite(duration) && duration > 0)
          ? Math.min(Math.max(0, resumeHint), Math.max(0, duration - 0.25))
          : Math.max(0, resumeHint);
    ...
    if (keepPlaybackState && wasPlaying) {
      ...
      audio.play().then(() => {
        ...
        animateMusicBars(true);
        updateSongInfo();
        if (!audio.muted) fadeAudioVolume(audio, 0, 0.2, 520);
      }).catch(() => {
        // If blocked, keep state; gesture handler will recover.
        audio.volume = prevVolume;
      }).finally(() => {
        state.switchingTrack = false;
      });
      return;
    }
  ...
  // Smooth transition when switching while playing.
  if (wasPlaying && keepPlaybackState) {
    const startVol = audio.muted ? 0 : audio.volume;
    fadeAudioVolume(audio, startVol, 0, 220);
    setTimeout(() => {
