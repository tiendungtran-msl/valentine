...
const CONFIG = {
  ...
  music: [
    // { title: 'Tên bài hát', file: 'song.mp3' },
  ],

  // Lời tỏ tình trên Landing page
  typingText: 'Anh chưa chuẩn bị được cho em một lời tỏ tình chỉnh chu…\nnên hôm nay, anh xin chiếm lấy lời nói "yêu em",\n"thương em nhất trên thế giới này"…\nđể dành cho em. 💕',
...
const LOVE_THINGS = [
  'Cách em cười — khiến cả thế giới anh sáng bừng',
  'Cách em giận dỗi — đáng yêu không thể tả',
  'Cách em quan tâm anh bằng những điều nhỏ nhất',
  'Cách em khiến tim anh loạn nhịp mỗi khi nhìn em',
  'Đôi mắt em — sâu thẳm như bầu trời đêm',
  'Giọng nói em — dịu dàng hơn mọi giai điệu',
  'Nụ cười em — xóa tan mọi mệt mỏi của anh',
  'Cách em nhắn tin cho anh mỗi sáng',
  'Cách em lo lắng khi anh ốm',
  'Cách em kể chuyện — anh có thể nghe mãi',
  'Mái tóc em — mềm mại và thơm lắm',
  'Cách em cắn môi khi suy nghĩ',
  'Cách em tựa vào vai anh',
  'Cách em nhìn anh — khiến anh quên mọi thứ',
  'Sự dũng cảm ẩn sau vẻ ngoài dịu dàng của em',
  'Cách em kiên nhẫn với mọi người',
  'Cách em yêu thương gia đình',
  'Cách em cố gắng mỗi ngày — anh khâm phục em',
  'Cách em khiến anh muốn trở thành người tốt hơn',
  'Bàn tay nhỏ nhắn của em — anh muốn nắm mãi',
  'Cách em say sưa kể về đam mê của mình',
  'Cách em ăn — đáng yêu vô cùng',
  'Cách em ngồi im nghe anh nói',
  'Cách em xoa đầu anh khi anh buồn',
  'Sự chấn thành trong từng lời em nói',
  'Cách em cười toe toét khi vui',
  'Cách em mím môi khi giận',
  'Trái tim ấm áp và nhân hậu của em',
  'Cách em luôn tin tưởng anh',
  'Cách em khóc khi xem phim buồn — anh thấy thương',
  'Những tin nhắn dài em gửi lúc đêm khuya',
  'Cách em chăm sóc bản thân — anh yên tâm lắm',
  'Cách em dỗ dành anh khi anh căng thẳng',
  'Sự thông minh và sắc sảo của em',
  'Cách em nhìn ra ngoài cửa sổ — mơ màng và đẹp',
  'Hương thơm của em — anh nhớ mãi',
  'Cách em cười phá lên — thoải mái và hạnh phúc',
  'Sự ấm áp em mang đến cho cuộc sống của anh',
  'Cách em luôn nghĩ cho người khác trước mình',
  'Cách em đặt trái tim vào mọi việc em làm',
  'Đôi má em — hồng lên khi xấu hổ',
  'Cách em vươn vai mỗi sáng — đáng yêu lắm',
  'Sự kiên cường và mạnh mẽ bên trong em',
  'Cách em cùng anh đi qua những ngày khó khăn',
  'Cách em nhắc anh uống nước, ăn cơm',
  'Cách em làm mọi thứ trở nên tốt đẹp hơn',
  'Cách em là chính em — không cần giả vờ',
  'Mọi khoảnh khắc bên em — đều là khoảnh khắc đẹp nhất',
  'Cách em khiến anh tin vào tình yêu',
  'Tất cả mọi thứ ở em — anh yêu hết, yêu trọn vẹn 💕',
];
...
function initMelody() {
  ...
  // Set up first song if available
  if (CONFIG.music.length > 0) {
    audio.src = `assets/music/${CONFIG.music[0].file}`;
    songInfo.textContent = CONFIG.music[0].title || 'Nhấn play để nghe…';
  } else {
    songInfo.textContent = 'Thêm nhạc vào /assets/music/ 🎵';
  }
