...
const CONFIG = {
  ...
  music: [
    ...
    },
    {
      ...
      // Nếu bài có đoạn dạo nhạc trước khi vào lời, tăng giá trị này (giây) để lyric KHÔNG bị chạy nhanh hơn nhạc.
      // Ví dụ: lyricOffset: 8 nghĩa là audio tới 8s mới bắt đầu tính lyric ở mốc 0s.
      lyricOffset: 8,
      lrcFile: 'until-i-found-you.lrc',
      lyrics: [
        { time: 0, text: '♪ Until I Found You' },
        { time: 8, text: 'Thêm lyric trong file assets/lyrics/until-i-found-you.lrc' },
        { time: 18, text: 'Hoặc dùng mảng lyrics: { time: giây, text: "..." }' },
      ],
    },
    {
      title: 'Có Đôi Điều',
...
const state = {
  ...
  memoryStageLines: [
    ...
    'Anh đi qua nhiều nơi, nhưng bình yên nhất vẫn là lúc được nghe em kể chuyện thường ngày.',
    'Có những tối chỉ cần em nói “hôm nay em ổn”, lòng anh cũng nhẹ đi rất nhiều.',
    'Đôi khi tình yêu chỉ là chờ nhau online, hỏi nhau đã ăn chưa, rồi ngủ ngon nhé.',
    'Anh không giỏi hứa điều lớn, nhưng anh luôn muốn làm tốt những điều nhỏ cho em.',
    'Mỗi tấm ảnh là một lần anh chắc hơn rằng: chọn em là điều đúng nhất anh từng làm.',
    'Nếu mai có mệt, mình cứ nắm tay nhau chậm lại — miễn là vẫn cùng một hướng.',
  ],
};
...
function getCurrentTrackLyrics() {
  ...
  const parsedLyrics = Array.isArray(song.parsedLyrics) ? song.parsedLyrics : [];
  const inlineLyrics = Array.isArray(song.lyrics) ? song.lyrics : [];
  const source = parsedLyrics.length > 0 ? parsedLyrics : inlineLyrics;
...
async function loadTrackLyrics(trackIndex) {
  ...
  if (Array.isArray(song.parsedLyrics) && song.parsedLyrics.length > 0) return;

  try {
    const response = await fetch(`assets/lyrics/${song.lrcFile}`);
    if (!response.ok) return;
    const raw = await response.text();
    song.parsedLyrics = parseLrc(raw);
  } catch (error) {
    console.warn('Không tải được file lyric:', error);
  }
}
